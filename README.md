...
> [!WARNING]
> This client runs on FHEVM v9 (TFHE-RS 1.4.x and KMS 0.12.x).
...
Retrieve the forked TFHE-RS repo and build it with the c-api feature. The fork just adds the "safe" serialization of `ProvenCompactCiphertextList`. (I lost so many hours figuring out that "safe serialization" was absolutely different from "serialization")
The branch commit is based on the tfhe-rs `release/1.4.x` branch.
```bash
$ git clone https://github.com/geoxel/tfhe-rs.git
$ cd tfhe-rs
$ git checkout geoxel/safe_serialization_v9
...
The commit is based on the KMS `release/v0.12.x` branch.
```bash
...
Retrieving FHECounter contract 0x097b013eb3F9811e11AEE4c57E032EE4EdF2CC7c...
